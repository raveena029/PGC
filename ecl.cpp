    ...
    do {
        ...
        for (int v = 0; v < nodes; v++) {
            ...
            if (__builtin_popcount(pcol) > 1) {
                ...
                pcol = (1 << smallestColor);



                again |= (color[v] != __builtin_clz(posscol[v]));
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        ...
        printf("USAGE: %s input_file_name thread_count\n\n", argv[0]);
    ...
    if (threads < 1) {
        ...
        fprintf(stderr, "ERROR: thread_count must be at least 1\n");
